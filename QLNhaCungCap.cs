...
namespace QLKhoHang
{
    public partial class QLNhaCungCap : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                txtMaNCC.Text = Convert.ToString(dataGridView1.CurrentRow.Cells["clMaKho"].Value);
                txtTenNCC.Text = Convert.ToString(dataGridView1.CurrentRow.Cells["clTenKho"].Value);
                txtDiaChi.Text = Convert.ToString(dataGridView1.CurrentRow.Cells["clDiaChi"].Value);
                txtSDT.Text = Convert.ToString(dataGridView1.CurrentRow.Cells["clSDT"].Value);
            }
        }
        ...
        private void bntLuu_Click(object sender, EventArgs e)
        {
            ...
            if (c2 + 1 < 10)
            {
                txtMaNCC.Text = "CC110" + (c2 + 1).ToString();
            }
            else if (c2 + 1 < 100)
            {
                txtMaNCC.Text = "CC11" + (c2 + 1).ToString();
            }

            if (txtTenNCC.Text.Trim() == "")
            {
                MessageBox.Show("Hãy nhập tên nhà cung cấp", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ActiveControl = txtTenNCC;
                return;
            }
            if (txtDiaChi.Text.Trim() == "")
            {
                MessageBox.Show("Hãy nhập địa chỉ của nhà cung cấp", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ActiveControl = txtDiaChi;
                return;
            }
            if (txtSDT.Text.Trim() == "")
            {
                MessageBox.Show("Hãy nhập số điện thoại của nhà cung cấp", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("select * from NhaCungCap where TenNCC like '" + "%" + txtSearch.Text + "%'", conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
            dataGridView1.DataSource = dt;
            conn.Close();
